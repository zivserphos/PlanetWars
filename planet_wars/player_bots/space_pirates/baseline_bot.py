...
class Firstroundstrategy(Player):
    ...
    def ships_to_send_in_a_flee(self, source_planet: Planet, dest_planet: Planet) -> int:
        future_enemy_size = dest_planet.num_ships + dest_planet.growth_rate*Planet.distance_between_planets(source_planet, dest_planet)
        return future_enemy_size+Firstroundstrategy.EXTRA

    def play_turn(self, game: PlanetWars) -> Iterable[Order]:
        Firstroundstrategy.TURNS+=1
        """
        ...
        for i in planets_to_attack:
            if i.num_ships < my_strongest_planet.num_ships:
                ...
                planet_distances[i.planet_id]=score
        enemy_or_neutral_weakest_score_id = max(planet_distances.keys(), key=lambda planet: planet_distances[planet])
        return [Order(
            my_strongest_planet,
